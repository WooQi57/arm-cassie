...
mjvFigure figsensor;
//Arm
#define NUM_ARM 6 //arm
static cassie_arm mj_arm[NUM_ARM];
...
double* cassie_forceandtorque(cassie_sim_t *c)
{
    static double out[6];
    mju_copy_fp(out, &c->d->sensordata[29], 6);
...
void arm_cassie_sim_upright(cassie_sim_t *c)
{
    c->d->qpos[0]=0;
    c->d->qpos[1]=0;
    c->d->qpos[2]=2;
...
/*******************************************************************************
 * Cassie arm simulation function
 ******************************************************************************/
void arm_set_gain(double* target, double* pgain, double* dgain)
{
    for (int arm = 0; arm < NUM_ARM; arm++)
    {
        // mju_copy_fp(&mj_arm[i].target,&target[i],1);
        // mju_copy_fp(&mj_arm[i].pGain,&pgain[i],1);
        // mju_copy_fp(&mj_arm[i].dGain,&dgain[i],1);
        mj_arm[arm].target=target[arm];
        mj_arm[arm].pGain=pgain[arm];
        mj_arm[arm].dGain=dgain[arm];
    }
}

void arm_set_torque(double* torque)
{
    for (int arm = 0; arm < NUM_ARM; arm++)
    {
        mj_arm[arm].torque=torque[arm];
    }
}

void arm_torque(cassie_sim_t *c)
{
    // printf("*********************\n");
    for (int i = 0; i < NUM_ARM; i++)
    {
        double t;
        if (mj_arm[i].pGain==0 && mj_arm[i].dGain==0)
        {
            t = mj_arm[i].torque;
        }
        else
        {
            t=(mj_arm[i].target - c->d->qpos[35+i])*mj_arm[i].pGain-c->d->qvel[32+i]*mj_arm[i].dGain;
        } 
        // mj_arm[i].torque = (mj_arm[i].target - c->d->qpos[35+i])*mj_arm[i].pGain-c->d->qvel[32+i]*mj_arm[i].dGain;
        // printf("No.%d pos=%f, vel=%f, target= %f, torque=%f\n", i+1, c->d->qpos[35+i], c->d->qvel[32+i], mj_arm[i].target, mj_arm[i].torque);
        // printf("No.%d torque=%f\n", i+1,  t);
        // printf("Err:%f\n", mj_arm[i].target - c->d->qpos[35+i]);

        mju_copy_fp(&c->d->ctrl[NUM_DRIVES+i], &t, 1);
    }    
}

void arm_set_gripper(cassie_sim_t *c, bool close)
{
    if (close)
        c->d->ctrl[NUM_DRIVES+NUM_ARM]=1;
    else
        c->d->ctrl[NUM_DRIVES+NUM_ARM]=0;
}
