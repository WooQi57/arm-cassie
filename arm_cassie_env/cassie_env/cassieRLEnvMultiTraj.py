from cassie_m.cassiemujoco_ctypes import *
import math
...
class cassieRLEnvMultiTraj(cassieRLEnvMultiSkill):
	def __init__(self,visual):
		...
		self.speed = 0
		self.side_speed = 0
		self.observation_space = spaces.Box(low=-np.inf,high=np.inf,shape=(85,))
	...
	def get_kin_state(self):
		if self.speed < 0:
			...
			vel[0] *= self.speed
		#print("vel", vel[0])
		pose[1] = self.side_speed * (self.counter*self.max_phase+self.phase)*self.control_rate*0.0005
		pose[3] = 1
		pose[4:7] = 0
		pose[7] = 0
		pose[8] = 0
	...
	def get_kin_next_state(self):
		if self.speed < 0:
			...
			pose[0] += (self.trajectory.qpos[1681, 0]- self.trajectory.qpos[0, 0])* counter * self.speed
			#print(pose[0])
			pose[1] = 0
			vel[0] *= self.speed
			pose[1] = self.side_speed * (counter*self.max_phase+self.phase)*self.control_rate*0.0005
