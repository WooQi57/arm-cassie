...
class cassieRLEnv:
	def __init__(self,visual):
		self.model = "./cassie_m/model/cassie.xml"
...
class cassieRLEnvDelay(cassieRLEnv):
	def __init__(self,visual):
		self.model = "./cassie_m/model/flatcassie_box.xml"
		self.sim = CassieSim(self.model)
		self.visual = visual
		if self.visual:
			self.vis = CassieVis(self.sim)
		self.observation_space = spaces.Box(low=-np.inf,high=np.inf,shape=(80,))
		self.action_space = spaces.Box(low=np.float32(np.array([-1]*10)), high=np.float32(np.array([1]*10)))
		self.trajectory = CassieTrajectory("trajectory/stepdata.bin")
		self.P = np.array([100, 100, 88, 96, 50, 100, 100, 88, 96, 50])
		self.D = np.array([10.0, 10.0, 8.0, 9.6, 5.0, 10.0, 10.0, 8.0, 9.6, 5.0])
		self.u = pd_in_t()
