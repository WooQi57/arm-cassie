...
class cassieRLEnvStable(cassieRLEnvMirror):
    def __init__(self,visual=False,setforce=-1):
        ...
        self.action_space = spaces.Box(low=np.float32(np.array([-3.14/3]*10+[-0.1]*6)), high=np.float32(np.array([3.14/3]*10+[0.1]*6)))
        self.observation_space = spaces.Box(low=-np.inf,high=np.inf,shape=(97,))
    ...
    def step_simulation(self, action):
        ...
        # print(action[10:])
        self.arm_setup_pos = [self.arm_zero[i]+action[10+i] for i in range(6)]
    ...
    def reset(self):
        if self.time != 0 :
            ...
            self.rew_ee_buf = self.rew_ee / self.time
        ...
        # set force
        self.zforce = 0.2*random.randint(0,10)-1+50
        if self.setforce != -1:
            self.zforce = self.setforce
            
        
        if self.visual:
            pass
            # print("z-axis force:",self.zforce)
    ...
    def reset_test(self):
        ...
        qpos=np.concatenate([qpos,arm_zero],axis=0)
        # print(qpos)
        self.sim.set_qpos(qpos)
        self.sim.set_qvel(qvel)
        self.cassie_state = self.sim.step_pd(self.u)
    ...
    def get_state(self):
        ...
        if self.phase < 14:
            ...
            arm_joint_position = np.zeros(6)
            arm_joint_velocity = np.zeros(6)
            arm_joint_position = np.copy(self.sim.qpos()[-6:])
            arm_joint_velocity = np.copy(self.sim.qvel()[-6:])
            return np.concatenate([useful_state, ref_pos[pos_index], ref_vel[vel_index],  arm_joint_position, arm_joint_velocity])
        else:
    ...
    def compute_reward(self,action):
        ...
        arm_height = self.sim.xpos("ee")
        arm_height_penalty = (arm_height[2]-1.5)**3
        ...
        orientation_penalty = roll**2+pitch**2+(yaw - self.orientation)**2
        ...
        total_reward = 0.7*np.exp(-joint_penalty)       \
                            +0.2*np.exp(-com_penalty)       \
                            +0.2*np.exp(-10*orientation_penalty)   \
                            +0.05*np.exp(-hip_action_penalty)  \
                            -0.0001*self.acc/self.time    \
                            ...
                            +0.1*np.exp(-spring_penalty)  \
                            -10*self.termination \
                            # +0.3*np.exp(-arm_height_penalty)
                            # +0.01*np.exp(-action_penalty)#-0.3*self.body_acc/self.time
        ...
        self.rew_ee += 0.3*np.exp(-arm_height_penalty)
